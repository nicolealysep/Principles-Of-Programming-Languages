...
class OperatorTests extends AnyFunSuite {
    ...
    test("Simple Triangle"){
        ...
        v1 match {
            case FigValue(canvas) => {
                val tri  = new Polygon(List((-50,0), (50,0), (0, 86.60255)))
                ...
                assert (coordinates.contains((-50,0)), "Triangles must have (0,0) ")
                assert(coordinates.contains(50,0), "Triangle must have (100,0)")
                assert(coordinates.count( distance(_, (0, 86.60254)) <= 1E-04) == 1, "Triangle must have a point very close to (50.0,86.60254037844386)")
            }
            case _ => assert(false, "Program evaluates to the wrong type. Expected value type is FigValue. Instead we got ${v1}")
        }
    }
    ...
    test("Simple Rectangle"){
        ...
        v1 match {
            case FigValue(canvas) => {
                val expectedVerts = List((-10,-10), (10, -10), (10, 10), (-10, 10))
                assert (canvas.numCircles == 0)
                assert (canvas.numPolygons == 1)
    ...
    test("Simple Line"){
        ...
        v1 match {
            case FigValue(canvas) => {
                val expectedVerts = List((-5,0), (5, 0))
                assert (canvas.numCircles == 0)
                assert (canvas.numPolygons == 1)
                val myrectangle = canvas.getListOfObjects.head.asInstanceOf[Polygon]
                val coordinates = myrectangle.cList
        ...
        v1 match {
            case FigValue(canvas) => {
                ...
                assert(listOfObjects.forall({
                    ...
                    case Polygon(lst)=> {
                        val expectedCoords = List((-12.5,-12.5), (12.5, -12.5), (12.5, 12.5), (-12.5, 12.5))
    ...
    test("Place Right Operator Test") {
        ...
        v1 match {
            case FigValue(canvas) => {
                ...
                assert(listOfObjects.forall({
                    case MyCircle(c, r) => {
                        assert (math.abs(c._1 - 37.5)<= 1E-6 && math.abs(c._2) <= 1E-6, s"Center of circle wrong: expected (50,25), got $c")
                        assert ( math.abs(r - 25.0) <= 1E-6, "Radius of circle wrong: expected 25, got $r")
                        true
                    }
                    case Polygon(lst)=> {
    ...
    test("Place Top Test") {
        ...
        v1 match {
            case FigValue(canvas) => {
                ...
                assert(listOfObjects.forall({
                    case MyCircle(c, r) => {
                        assert (math.abs(c._1  )<= 1E-6 && math.abs(c._2 -30 ) <= 1E-6, s"Center of circle wrong: expected (0,30), got $c")
                        assert ( math.abs(r - 20.0) <= 1E-6, "Radius of circle wrong: expected 25, got $r")
                        true
                    }
                    case Polygon(lst)=> {
                        val expectedCoords: List[(Double, Double)] = List((-10.0,-10.0), (10.0,-10.0), (10, 10), (-10.0, 10.0))
                        assert (lst.length == 4, "Rectangle must have 4 coordinates")
                        lst.forall( c => expectedCoords.count(p => math.abs(p._1 - c._1) <= 1E-06
                          && math.abs(p._2 - c._2) <= 1E-06) == 1
                        )
